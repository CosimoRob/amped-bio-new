...
Status: overview for review. Build Board items #22, #23, #24. Editable copy: [Fan Loyalty: Overview (Docs)](https://claude.ai/code/artifact/a9e65a11-3272-4ac3-86e5-38a4666f8a78).
Owner: Rob Frasca. Drafted by Claude, 2026-09-26.

This overview replaces two Lovable prototypes (`CosimoRob/revolution-rewards-hub`, `CosimoRob/revo-superfans`). They are rebuilt natively in Amped. No separate user base, wallet or database. The detailed spec follows once the open decisions in section 6 are settled.
...
- Follow button on every bio. One tap for signed-in users. Visitors create a full Amped account to follow (Rob's decision, 2026-09-26). This replaces the earlier email-only recommendation.
...
- **Visitor follow:** a full Amped account is required to follow. Rob chose this over email-only follow.
- **Follower visibility:** the creator sees the full list; the public sees only the count; a fan can opt in to appear publicly.
- **Accepted recommendations:** points expire after 12 months without activity; raffles allowed with counsel-reviewed rules; tiers default to Fan, Core, Inner Circle; staking counts toward Fan Score only; seasons are quarterly; identity level multiplies Fan Score.
...
- **One-line pitch:** "Your bio knows your biggest fans." Amped is the only link-in-bio where fans earn status and perks from the creators they follow.
- **For creators:** a fan CRM, tiers and a perk shop built into the page they already share. No separate loyalty app, no token launch.
- **For fans:** a Superfan Passport that follows them across creators and shows what they have unlocked.
- **Against Linktree and Beacons:** they reward creators; Amped rewards fans. Neither has a fan score or a fan-side balance.
- **Against quest platforms (Galxe, Zealy, Layer3):** only first-party verified actions, so no paid X engagement and no bot farms.
- **Launch hooks:** seed 10 to 20 creators running a first season; make Inner Circle badges shareable cards; add brand-sponsored perks through the brand portal (#7).
- **Language rules:** no "earn", "yield" or "APR" around staking. Points have no cash value and are not transferable.
...
- #11 analytics: follow, quest, redemption and tier events go through the shared event layer.
...
## 9. Counsel review before launch
