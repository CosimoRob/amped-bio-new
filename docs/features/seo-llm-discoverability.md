...
- It shows the creator photo (circle, 220 px) when present, the name, `@handle`, and the first line of the bio.
- If the photo fails to load, the image falls back to the handle's first letter.
...
- `sameAs` lists the URLs of `link` blocks whose platform is a social network: twitter, instagram, tiktok, linkedin, youtube, facebook, github, telegram, discord, medium, mirror, warpcast, zora, lens, patreon, substack or twitch. `email`, `document` and `custom` links are excluded.
- Email is never included.
- JSON is serialized with `<` escaped, to prevent script injection from user text.

On the home page:

- `WebSite` with `name`, `url` and `potentialAction` omitted, since there is no site search.
...
- `getSitemapEntries({ page, pageSize })` returns `[{ handle, lastModified }]`.
  ...
  - `lastModified` is the later of `user.updated_at` and the newest `block.updated_at` for that user, falling back to `created_at`.
...
This filter lives in one shared function so the sitemap and the page `robots` decision cannot drift apart. The page gets the flag from a new `indexable` boolean on the `getHandle` output.

### 3.6 Sitemaps

| Path | Source | Contents |
|---|---|---|
| `/sitemap.xml` | `src/app/sitemap.ts` | `/`, `/i/pools`, `/i/blog`, `/i/network`, blog posts (latest 200), public pool detail pages |
...
- Content:
  ...
  - Short prose on what an Amped bio is and the `/@handle` URL pattern
  - H2 sections with absolute links: Key pages (home, pools, blog, network), Creator pools, Sitemaps
  - A review date
...
5. Google Rich Results Test passes on a sample bio.
6. `pnpm run typecheck` passes for server and landingpage, and `pnpm run build` passes for landingpage.

### 3.12 After deploy (Rob)

1. Verify `amped.bio` in Google Search Console and Bing Webmaster Tools.
2. Submit `https://amped.bio/sitemap-index.xml` to both.
3. Track indexed pages and bio impressions weekly for 4 weeks.
